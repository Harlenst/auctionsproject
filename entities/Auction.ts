...
export class Auction {
    ...
    placeBid(bid: Bid): void {
        ...
        if (this.bids.length === 0 && bid.amount < this.basePrice) {
            throw new Error('The first bid must be greater than or equal to the base price');
        }
        if (this.bids.length > 0) {

            const highestBid = this.bids[this.bids.length - 1];

            const minimumAmount = highestBid.amount + this.minimumIncrement;
        ...
        if (this.bids.length > 0) {
            ...
            if (highestBid.userId === bid.userId) {
                throw new Error('The highest bidder cannot bid again');
            }
        }
    }
}